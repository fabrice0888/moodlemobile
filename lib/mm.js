...
/*
    getFormattedLog: function(filter) {
     ...
     getFormattedLog: function(filter) {
        ...
        MM.db.each("mmStats", function(el){
            ...
            logInfo = logInfo +   el.id+","+el.username+","+el.sectionId+","+el.courseId+","+el.contentId+","+el.automated+","+el.url+","+
                     el.filename+","+el.time+","+el.date+","+el.timestamp+","+el.lastaccesstimestamp+","+el.lastaccessdate+ "<br />";

           }); 

         MM.db.each("mmEventStats", function(el){
            el = el.toJSON();
 
            logInfo = logInfo + el.id+","+el.linkAvailableCount+","+el.linkNonAvailableCount+","+el.NoConnectivityPeriod+ "<br />";
    ...
    showLog: function(filter) {
        ...
        logInfo += '<div class="centered"><a href="mailto:fabrice.tonta@gmail.com?subject=MMLog&body=' + mailBody + '"><button>' + MM.lang.s("email") + '</button></a></div>';
